...
| `background.js` | SW:native port 管理 + 自动重连 + 请求分发到 content script + 白名单 + screenshot 代理 + **page_snapshot_precise**(chrome.debugger + CDP 链路) |
| `content.js` | DOM 遍历(snapshot)/click/fill/scroll/wait + Toast UI + 脱敏;**page_eval**(Function 构造器执行 + 序列化 + 脱敏 + 放大版 Toast);**动态注入**(非 manifest content_scripts) |

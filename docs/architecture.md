...
扩展源码用 **TypeScript**(strict)写在 `extension/src/*.ts`,由 **esbuild** 打包成 IIFE 到 `extension/dist/`,静态资源(manifest/HTML/CSS/icons)一并拷入;**load-unpacked 目标是 `extension/dist/`**(不是 `extension/`)。改代码须先 `npm run build`(或 `just ext-build`)。详见 [ADR-0012](./adr/0012-typescript-esbuild-extension-build.md)。
...
扩展本身以 **load-unpacked** 方式从 **`extension/dist/`**(esbuild 构建产物,由 `src/*.ts` 打包 + 静态资源拷贝而来)加载,`install.sh` 会先构建再加载。dist/ 不入库,克隆后须先 `npm run build`(或 `just ext-build`)。详见 [ADR-0012](./adr/0012-typescript-esbuild-extension-build.md)。
...
| 工程门禁 | justfile + GitHub Actions | 统一任务入口 + CI(fmt/clippy -D warnings/eslint/prettier + 测试);Cargo 为版本单源。详见 [ADR-0013](./adr/0013-ci-and-toolchain.md) |
...
- **[ADR-0013](./adr/0013-ci-and-toolchain.md)**:justfile 任务入口 + GitHub Actions CI + rustfmt/clippy/eslint/prettier 门禁 + Cargo 为源的版本同步。

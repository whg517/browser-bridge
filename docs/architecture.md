...
| 文件 | 职责 |
|------|------|
| `main.rs` | 模式分发:无参数=MCP server,`--native-host`=native host,`doctor`/`status`=只读自检(见 [cli.md](./cli.md)),`--help`=帮助 |
...
| `tools/` | 15 个工具的 schema 定义(`catalogue.rs`)+ `HANDLERS` 注册表(`{name, build_payload}` 纯函数,`mod.rs`)+ 参数塑形(`handlers.rs`)→ dispatch 到 session.call |
| `session.rs` | 连接管理 + 请求/响应按 id 配对(mpsc channel per id)+ 每连接 generation id(修 writer-clobber 竞态、断开即 `Disconnected` 排空)+ 120s 超时 |
...
- **加新工具**:在 `tools/catalogue.rs` 加 schema 定义 + 在 `tools/mod.rs` 加一条 `HANDLERS` 记录(`build_payload` 纯函数),扩展 background/content 加对应 op 处理
...
> 运行时排查这两条链路(连接是否可达、锁文件/端口/manifest 是否就位)可用只读的
> `browser-bridge doctor`,见 [cli.md](./cli.md)。

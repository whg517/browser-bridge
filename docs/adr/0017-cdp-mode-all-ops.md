...
- **Confirmation without a content script** *(removed — see [ADR-0020](./0020-remove-interactive-confirmations.md))*: CDP mode originally rebuilt the high-risk-click and `page_eval` confirmation toasts inside the page via `Runtime.evaluate`, matching the content path. Those per-action confirmations were removed in ADR-0020, so CDP mode no longer builds them. The `pageEvalEnabled` kill switch, `evalMask` redaction, and the allowlist check still match the content path.

...
>
>**执行状态**:P0 治理底座已完成;P1、P2 的验收标准已**实质达成**(契约单源、判别联合、
>错误码、Tool Registry + 目录拆分、Policy 层、连接 generation 竞态修复、tag 驱动多平台发布、
>扩展 zip、SBOM/provenance、`doctor`/`status`、结构化日志/审计与 request/connection id、nightly)。
>剩余项均属「有意延后 / 评估为非必需 / 需真实浏览器」三类,逐项见 §12 标注与 **§16 完成度评估**;
>另见提交历史与 `GOVERNANCE.md`。
>
>**状态图例**:✅ 已落地 / ⚠ 部分 / ⬜ 未做。
...
3. ✅ 已落地(TS 侧) `BridgeReq` 判别联合——由契约生成 `BridgeCommand`(#25),验收标准「TS 无法构造非法请求」已达成。Rust tagged-enum 半边经评估为**非必需**(客户端可信、扩展侧已强类型、契约测试已覆盖),不再推进,详见 §16。
4. ✅ 已落地 统一跨进程错误码(`errors.json` + `CallError.code`,`cargo test` 校验映射)。
5. ⚠ 部分 / **有意延后** 协议和能力版本握手:契约(`protocol-version.json` + `capabilities.json`)已定义,代码侧 wiring 未接。经评估当前**低紧迫**(仅单一协议版本、安装同时打包二进制与扩展、无版本偏斜),且端到端验证需真实浏览器。**触发条件**:当二进制与扩展可独立升级(如上架 Web Store 或分离发布节奏)时再接线。详见 §16 与 [compatibility.md](./compatibility.md)。
...
9. ⚠ 部分 将 Session 改为显式连接状态机:RFC-0001 首阶段的 generation-guard 已落地——验收标准「旧连接无法清除或影响新连接」**已达成**(竞态已修 + pending drain)。完整 5 态枚举经评估为**非必需**(不修复已知缺陷、单连接场景、验证仍需真实浏览器),不再推进,详见 §16。
...
2. ✅ 已落地 生成预编译二进制和 Extension zip:每平台归档(macOS arm64 / Linux x64 tarball + Windows x64 zip,#23);独立 `browser-bridge-extension-*.zip` + `.sha256` + provenance 也已产出(#22)。
...
# 十六、治理完成度评估(2026-07-14)

本节回答"路线图是否推进完",并记录对少数剩余项"是否有必要"的评估结论。

## 结论:P0–P2 的验收标准已实质达成

| 阶段 | 验收达成 | 说明 |
| --- | --- | --- |
| P0 治理底座 | ✅ 全部 | GOVERNANCE/SECURITY/威胁模型、模板、分支保护、toolchain、Dependabot、audit/deny、SHA 固定、风险矩阵均已落地。 |
| P1 协议与结构 | ✅ 6 项验收中 5 项达成,1 项有意延后 | 单源契约 ✅、TS 无法构造非法请求 ✅、稳定错误码 ✅、旧连接不影响新连接 ✅、新增工具局部化 ✅;唯一未达成的「版本不匹配快速失败」= P1#5,已有意延后(见下)。 |
| P2 发布与运维 | ✅ 实质达成 | 免手工构建发布 ✅、可诊断安装/版本 ✅、失败请求可经 request/connection id 定位 ✅、发布物来源可验证(checksum+SBOM+provenance)✅;「新版本可判断兼容」依赖 P1#5 握手,同延后。 |
| P3 产品化 | N/A | 按设计仅在明确需求后进行;Windows/Linux 支持已落地,其余未启动。 |

§15 的五项最高优先级治理工作均已实质交付:契约单源 ✅、连接状态机(核心竞态修复)✅、安全治理制度 ✅、自动化发布 ✅、可观测与诊断 ✅。

## 剩余项分类:没有"待补的欠债",只有三类显式记录项

**① 评估为非必需(不再推进)**
- **P1#3 Rust tagged-enum**:验收已由 TS 判别联合达成;客户端可信 + 扩展已强类型 + 契约测试覆盖,将 `tools.rs` 重写为 `serde(tag/content)` 的边际收益近零、成本与风险高 → 不做。
- **P1#9 完整 5 态状态机**:核心竞态(writer-clobber + pending drain)已由 generation-guard 修复;形式化 5 态枚举不修复任何已知缺陷,属打磨 → 不做。

**② 有意延后(有明确触发条件)**
- **P1#5 版本/能力握手 wiring**:低紧迫(无版本偏斜),端到端需真实浏览器。触发:二进制与扩展可独立升级时。
- **P2#4 `upgrade` 子命令**:重跑 `install.sh` 即可升级,专用命令为便利项 → 按需。

**③ 需真实浏览器验证(交由维护者在隔离 Chrome 执行)**
- P1#11 重连/超时/迟到响应**集成**测试、P2#9 macOS 真实集成进入 release gate。按定义只能由驱动真实 Chrome 验证,不宜盲发。

**可选/按需(非阻塞,见 §14)**:§2.4 Node/Bun 统一、§2.2 增量 `any` 治理、§6.3 配置 schema 版本与迁移、§3.5 存储 Repository 模式。

## 一句话

工程治理中**可自动构建与验证的部分已推进完**;剩余全部归入上述三类,均已显式记录,不存在隐性技术债。后续投入应由**真实需求**(独立升级、企业化、新平台)或**真实浏览器验证**驱动,而非继续机械推进路线图。
